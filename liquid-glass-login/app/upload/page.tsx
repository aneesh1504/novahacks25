  ...
  return (
    <div className="min-h-screen w-full bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url('/images/gradient-background.jpg')" }}>
      ...
      <div className="relative max-w-5xl mx-auto px-6 pt-16 pb-10 z-10">
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-fuchsia-500 via-purple-500 to-indigo-600">Student ↔ Teacher Compatibility</h1>
        ...
        <div className="mt-10 rounded-2xl border border-white/70 bg-white/70 backdrop-blur-xl shadow-xl p-6 md:p-8">
          <div className="flex items-center gap-2 text-xl font-semibold text-slate-900"><Upload className="h-5 w-5 text-pink-600"/> Data Ingestion</div>
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-6">
            <div>
              ...
              <div className="mt-2 rounded-xl border-2 border-dashed border-slate-300 bg-white/70 p-4">
                ...
                <button onClick={()=>document.getElementById("teacher-input")?.click()} className="w-full py-2 rounded-lg bg-gradient-to-r from-indigo-600 to-fuchsia-600 text-white font-semibold shadow hover:opacity-95">Choose Teacher Files</button>
                <p className="mt-2 text-xs text-slate-600">{teacherFiles.length} selected</p>
              </div>
            </div>
            <div>
              <label className="flex items-center gap-2 font-semibold text-slate-900"><FileSpreadsheet className="h-4 w-4 text-pink-600"/> Student CSV</label>
              <div className="mt-2 rounded-xl border-2 border-dashed border-slate-300 bg-white/70 p-4">
                <input id="student-input" type="file" accept=".csv" onChange={(e)=>setStudentFile(e.target.files?.[0]||null)} className="hidden"/>
                <button onClick={()=>document.getElementById("student-input")?.click()} className="w-full py-2 rounded-lg bg-gradient-to-r from-pink-600 to-purple-600 text-white font-semibold shadow hover:opacity-95">Choose Student CSV</button>
                <p className="mt-2 text-xs text-slate-600">{studentFile ? studentFile.name : "None selected"}</p>
              </div>
            </div>
            <div>
              <label className="flex items-center gap-2 font-semibold text-slate-900"><RefreshCcw className="h-4 w-4 text-purple-600"/> Constraints</label>
          ...
          <button onClick={handleProcess} disabled={!ready || processing} className={`mt-8 w-full py-4 rounded-2xl font-semibold shadow-lg transition ${ready && !processing ? "bg-gradient-to-r from-indigo-600 to-pink-600 text-white" : "bg-slate-200 text-slate-500"}`}>
